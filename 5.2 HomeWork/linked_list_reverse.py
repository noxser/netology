...
class LinkedList:
    ...
    в нем же и создаем наш список из полученных данных
    '''
    ...
    def __init__(self, values: Iterable):  # получаем дланные
        previous = None
        self.head = None
        for value in values:  # в цикле создаем узлы и формируем ссылки вних
    ...
    # используется для вывода напечать при вызове print()
    def __iter__(self):
    ...
    def reverse(self):
        ...
        """
        Суть сего деяния в том чтобы переназначить ссылки у обьектов
        в обратном порядке.
        ...
        Цикл while будет работать по всем значениям списка
        начиная с заголовка N раз получаем сложность О(N)
        линейная зависимость от колличества элементов
        те операции что происходят в нутри цикла они в любом 
        случае выполняються и будт константой которой можно принебречь
        
        Поповоду потребления памяти могу сказать в сравнении с массивом
        связанный список более требовательный к памяти т.к для его хранения
        необходимо дополнителное место под ссылки для организации внутренней структуры.
        
        Потребление памяти данного алгоритма будет равно 1 т.к на вход он получает список
        и возврашает список тойже длины не создавая в результате работы каких либо 
        переменных и незаписывая их в список, все переменные временные и работают с полученными данными
        """
